...
nohup java -Xms5G -Xmx5G -XX:ReservedCodeCacheSize=256m -XX:MetaspaceSize=256m \
 ...
 -Dtype=$type -DfullNodes=$fullNodes -Dthread=$thread -DfilePath=$trxFilePath -Dqps=$qps -DmaxRows=$maxRows -DmaxTime=$maxTime \
 -DstartNum=$startNum -DendNum=$endNum  -jar Stress.jar >> sendtx.log 2>&1 &
